...
function changecolumnandtotal (id, kolfrominput)
{
							...
							$('[data-id='+id+'][data-type = 2]').val(tekkol); // поле количество строки таблицы меняем
...
// изменение суммы в соответствующей строке таблицы товаров						
...
function ftotalsum (strishod, strchange, $perr)
{
	...
	if (strishod.length > 1)  // на всякий случай, есть ли записи в исходном импуте (исходные кол-ва)
							...
							for (var y = 0; y < len; y++)  // идем по всем записям
							   {
								 ...
								 // alert ("gh");
								  if (flag1 == 1)  // если в инпуте с идами и измененными количествами по ним есть записи
								  {
									  
									len2 = arr2.length;  // это ранее преобразовали этот второй инпут в макссив
									//alert (len2);
					  ...
					  totals = totals + pr*kol; // в циклично прибавляем суммы
