...
**/				
    ...
    public function actionIndex()
    {
		...
		'categories3' => $categories3, 'categories4' => $categories4, 'buketi' => $buketi]);
    }
...
public function actionNoaja3() // вроде б не нужна
	{
		//$request = Yii::$app->request;
 ...
 public function actionAllbukets()
	{
		...
		$sessiondata = $session->get ($str9);
		$session->close();  */
		
		$flag_filterdata = 0;
		$flag_page = 0;
		$xx = 0; /* get параметром може приходить цена (и имею ввиду три интревала цены)
		и дело в том, что потом пользователь может вернуть цену в исходное положение (все параметры вернуть в исходное положение)
		а get параметр price в адресной строке будет так и висеть. И если не было отправки формы, но пришел гет парамерт прайс, то:
		если при этом нет гет параметра page (перелистывание страниц) то надо поменять в сессии строку (изменить интервал цены), так 
		как при перелистывании страниц данные фильтрации берутся из сессии. Если же есть гет параметр прайс (он там просто по факту висит в адресной строке) но при этом пришла форма
		то get параметр прайс никак не должен на что-то влиять
        Хотя пока отбой . И так есть условие if (($k == 1) or ($m == 1) )
		*/
		
		// $tt5 = Yii::$app->params['filt'];
		// echo "  333 "; echo $tt5;    echo "  333 ";
		$session->open();
		// в сессиях ниже хранятся хранится строка с набором параметров (цена, высота, ширина)
	/*	if ($unioncat)
		            {
			          $sess = "filt" . $unioncat; 
		            }
		else 
				   {
					   ...
					   $sess = "filtbase"; } // когда переключаются страницы, используется только get параметр. поэтому приходится работать с сессиями
		            }   */
					
					// следующие иф и елсе:
					// пользователь изменил параметры, отправил форму, эти параметры записались в сессию. далее листает страницы,
			//		все нормально, потом ушел на другие страницы сайта, опять пришел в allbukets. и при листании страниц будут 
			// опять вызываться даные из сессии, хотя так быть не должно. сессии надо обнулить, если нет параметра page
	/*	if 	($request->get('page') == NULL  )	{
			
			$dat = $session->remove ($sess);
			$sessiondata = NULL;
		
		}
		else {
			$sessiondata = $session->get ($sess);
			
		}   */
	//	$sessiondata = $session->get ($sess);
		$session->close();	
		
		if  (  $filterdata->load(Yii::$app->request->post())  ) { $flag_filterdata = 1; }
		if  (   $request->get('page') && strlen ($sessiondata) > 5    )  { $flag_page = 1; } // 5 - просто так
	//	if  (   $request->get('page')  ) { $t = Yii::$app->params['filt'];   }
		if (($flag_filterdata == 1) or ($flag_page == 1) )
		{
			
			
			
			
			if ($flag_filterdata == 1 ) // поступили данные из формы
			   {
				  // echo "5";
			      $filtmass = [];
				  ...
				  $xx = 1; // выше смотри описание что это
				 //  $tt = Yii::$app->params['filt'];
				 //  echo "  -- "; echo $tt;    echo "  -- ";
			   }
			  else // данные из формы всегда первичны. если даных не поступало, смотрим, есть ли get page и есть ли что-то в сессиях
			  // первичность формы поможет и в ситуации, когда в адресной строке висит get price , но пользователь изменил вручную цену - 
			  // - фильтрация будет идти по данным формы.
			  // а в случае перелистывания страниц (когда форма не отправляется) поможет следующее условие. (if $m == 1)
			  // которое опять же сработает, только если форма не приходила (в этом есть смысл). Благодаря этому условию если есть get параметр page (листание страниц),
			  // не будет произведена установки значения цен в соответствии с переданным парметром get price. Эта установка будет произведена лишь 
			  // в том случае, если не было данных форм и не было параметра get page. (эта установка выполняется в условии ниже)
			  {
			      if ( $flag_page == 1 )  // page передаются get параметром. поэтому тут работаем с сессиями
			        {
					   ...
					   // никаких установок не требуется. просто читаем данные из сессии и создаем на их основе фильтр. 
...
// следующие строки входят в условие if (($flag_filterdata == 1) or ($flag_page == 1) )
	//	если переданы данные формы, то в них будут и эти параметры, если есть что-то в сессиях, то там тоже будут эти парметры. 
		...
		if ($sort2 == 1)
			{
		      ...
		      $query = $query -> orderBy ([$ss => SORT_ASC]);
			
			}
		else 
			{
			
			  $query = $query -> orderBy ([$ss => SORT_DESC]);	
    ...
    $buketi = $query->offset($pages->offset)
		...
		else // если не приходило формы и если нету параметра get page (если есть get page но сессия почему-то пуста, то тоже сюда)
		{
			$request = Yii::$app->request;
		    $price = $request->get('price');
			$f = 1;
			if ($price)
			{
				...
				if ($price == 1)
				{
					$query = Buketi::find()
			          -> orderBy (['popul' => SORT_DESC])
			          -> where (['adjunct' => 0])
					  -> andWhere (['between', 'price', 0, 2000]);	
					$f = 0;  
					
					$filterdata->price1 = 1000;
			        $filterdata->price2 = 2000;
				}
				if ($price == 2)
				{
					$query = Buketi::find()
			          -> orderBy (['popul' => SORT_DESC])
			          -> where (['adjunct' => 0])
					  -> andWhere (['between', 'price', 2000, 4000]);	
                    $f = 0;	

                    $filterdata->price1 = 2000;
			        $filterdata->price2 = 4000;					
				}
				...
				if ($price == 3)
				{
					$query = Buketi::find()
			          -> orderBy (['popul' => SORT_DESC])
			          -> where (['adjunct' => 0])
					  -> andWhere (['between', 'price', 4000, 6000]);	
                    $f = 0;		

                    $filterdata->price1 = 4000;
			        $filterdata->price2 = 6000;					
				}
				
				if ($unioncat or $searchq)
		           $query = $query -> andWhere (['id' => $rows])	;
			...
			$query = Buketi::find()
			-> orderBy (['popul' => SORT_DESC])
			-> where (['adjunct' => 0]); // all здесь не надо
		...
		//	$query = $query -> andWhere (['between', 'price', 4000, 4500]);
